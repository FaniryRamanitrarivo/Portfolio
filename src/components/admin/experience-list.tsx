...
import { deleteExperience, reorderExperience } from "@/src/lib/actions/experience";

type OptimisticAction =
    | { type: "reorder"; ids: number[] }
    | { type: "delete"; id: number };

function reducer(state: ExperienceDTO[], action: OptimisticAction): ExperienceDTO[] {
    if (action.type === "delete") {
        return state.filter((s) => s.id !== action.id);
    }

    const byId = new Map(state.map((s) => [s.id, s]));
    return action.ids.map((id) => byId.get(id)).filter((s): s is ExperienceDTO => Boolean(s));
}

function formatPeriod(entry: ExperienceDTO): string {
    const format = (date: Date) =>
        new Date(date).toLocaleDateString("en-US", { month: "short", year: "numeric" });

    return `${format(entry.startDate)} - ${entry.endDate ? format(entry.endDate) : "Present"}`;
}
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="font-medium text-neutral-900 truncate">{entry.role}</div>
                <div className="text-sm text-neutral-500 line-clamp-1">
                    {entry.company} &middot; {formatPeriod(entry)}
                </div>
            </div>
...
export function ExperienceList({ entries }: { entries: ExperienceDTO[] }) {
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        const oldIndex = optimisticEntries.findIndex((s) => s.id === active.id);
        const newIndex = optimisticEntries.findIndex((s) => s.id === over.id);
        const nextIds = arrayMove(optimisticEntries, oldIndex, newIndex).map((s) => s.id);

        startTransition(async () => {
            applyOptimistic({ type: "reorder", ids: nextIds });
            try {
                await reorderExperience(nextIds);
                router.refresh();
            } catch {
    ...
    return (
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-neutral-900">Experience</h2>
                    <p className="text-neutral-600 mt-1">
                        Manage your professional background shown on /about — drag to reorder
                    </p>
                </div>
                <Link
            ...
            <div className="mt-8 space-y-3">
                {optimisticEntries.length === 0 ? (
                    <p className="text-neutral-500 text-center py-10">No experience entries found.</p>
                ) : (
                    <DndContext
                        sensors={sensors}
                    ...
                    >
                        <SortableContext
                            items={optimisticEntries.map((s) => s.id)}
